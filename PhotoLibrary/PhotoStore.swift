...
class PhotoStore {
    ...
    private func processPhotosRequest(data: Data?, error: Error?, completion: @escaping(PhotosResult)-> Void) {
        ...
        // in background task do make Photo instance
    ...
    func fetchInterestingPhotos(completion: @escaping (PhotosResult)-> Void) {
        ...
        let task = session.dataTask(with: request) {
            ...
            // result is [Photo]
            // var result = self.processPhotosRequest(data: data, error: error, completion: completion)
            self.processPhotosRequest(data: data, error: error, completion: { (result) in
                if case .failure = result {
                    completion(result)
                    
                    // Error Message Example: failure(Error Domain=NSURLErrorDomain Code=-1009 "The Internet connection appears to be offline."
                }
            })
